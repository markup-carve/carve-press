...
// `github:` or `git+` spec in `dependencies` means every install of this site
// generator clones carve-js and carve-grammars over git. Three consequences, all borne by the
...
//
// This is a REGRESSION guard, not a hypothetical, and this repository is where
// it went unnoticed longest. The published 0.1.1 tarball declares both engines
// as registry ranges; `main` carried `github:` specs for months afterwards, so
// the NEXT release would have been the one that started costing every consumer
// a clone. Nothing here read the specs - engine-drift.yml asserted their shape,
// but only on a schedule, and it wanted a git revision.
//
// The same regression has now happened in three sibling repos: carve-lsp put a
// `github:` spec back three and a half hours after the tag whose notes said
// consumers no longer needed Git, and markup-carve/carve-grammars#276 added
// this check after the same thing reached the registry there.
//
